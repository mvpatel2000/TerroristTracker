...
#input data
def load(ds):
   total = 0; #TODO: Total # of things
   terror = 0; #TODO # of terrorists
   ans = "0";
   for i in range(total):
      if i==terror:
         ans = "1"  
      inputText = open(location+"Stats/"+str(i)+'_'+ans+'_input.txt').read().split()
      input = [int(ipt) for ipt in inputText]
      ds.addSample(input, ans)
   return ds
...
def run(ds):
   net = buildNetwork(8, 1, bias=True, outclass = SigmoidLayer)
