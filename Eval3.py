...
test_image_list, test_label_list = read_labeled_image_list("eval3.txt")
...
train_input_queue = tf.train.slice_input_producer([images, labels], shuffle = False)
...
image_batch, label_batch = tf.train.batch([train_image, train_label],batch_size=17178) #21000
test_image_batch, test_label_batch = tf.train.batch([test_image, test_label],batch_size=17178)
...
for i in range(1):
  ...
  np.savetxt('frop3.txt',output.eval(feed_dict={x:test_imgs,y_:test_lbls}))
